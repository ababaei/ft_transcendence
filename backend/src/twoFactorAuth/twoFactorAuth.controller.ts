...
export class TwoFactorAuthController {
  ...
  @HttpCode(200)
  @UseGuards(JwtGuard)
  async turnOn2FA(@Req() req: Request, @Res() res: Response, @Body() { twoFaCode }: TwoFaCodeDto) {
  ...
  @HttpCode(200)
  @UseGuards(JwtGuard)
  async authenticate(@Req() req: Request, @Res() res: Response, @Body() { twoFaCode }: TwoFaCodeDto) {
    ...
    const token = await this.authService.signToken({ id: req.user['id'], twoFaAuthenticated: true })
